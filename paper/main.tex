...
task-success reporting. With the default Qwen3 thinking template, the model
spent the 96-token budget emitting reasoning text and produced zero parseable
actions. When the runtime disables thinking in the chat template for structured
action generation, the same model solves all three smoke tasks, with 3/3 parsed
actions and 3/3 successful executions. This supports a concrete runtime
requirement: rollout workers must control chat-template and decoding protocol
per action channel, not simply call a model endpoint and hope for a valid tool
call.
...
\caption{Qwen3-4B as a browser-action policy on MiniWoB contract tasks with
thinking disabled for structured JSON action generation.}
