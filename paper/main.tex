...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/failurebench_summary.tex}
}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/lifecycle_summary.tex}
}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/model_provenance_summary.tex}
}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/model_generation_summary.tex}
}
...
We also feed the measured action-channel summaries into the worker-pool
simulator, so the scheduling workload reflects real Qwen3-4B parse/success
rates and measured per-action latency rather than only hand-written synthetic
classes. Table~\ref{tab:model-action-throughput-results} reports the resulting
system consequence. On this empirical action-channel workload, retry-only
increases attempts from 200 to 245, raises failed cost, and reduces useful
throughput. Failure-aware execution does not claim additional task successes;
it converts typed invalid-action failures into replayable useful artifacts,
raising useful trajectories/hour from 4646.8 to 5995.8 while preserving the
same successful trajectory rate.

\begin{table}[t]
\centering
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/model_action_summary.tex}
}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/model_action_throughput_summary.tex}
}
\caption{Worker-pool throughput simulation parameterized by the measured
Qwen3-4B model-to-action summaries in Table~\ref{tab:model-action-results}.}
\label{tab:model-action-throughput-results}
\end{table}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/throughput_summary.tex}
}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/miniwob_contract_summary.tex}
}
